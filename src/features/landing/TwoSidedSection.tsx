...
export function TwoSidedSection() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    mm.add("(min-width: 768px)", () => {
      ...
      // Indicator and Right panel start prepared for coordinated entrance from right
      if (indicator) {
        gsap.set(indicator, {
          y: -6,
          autoAlpha: 0,
      ...
      // ── ENTRANCE TIMELINE: Nav and Right panel enter TOGETHER as section approaches viewport ──
      ...
      // ── PINNED TIMELINE: Interactive 2-sided convergence and match reveal ──
      ...
      // Step 2: Left enters from LEFT → RIGHT (0.10 to 0.35)
      tl.to(
        left,
        {
          xPercent: 0,
      ...
      // Step 3: Central seam, node, subtle magnetic pull (0.42 to 0.65)
      if (seam) {
        tl.to(
          seam,
          {
      ...
      // Step 4: Flip / Transformation into Verified Match (0.72 to 0.90)
      if (node) {
        tl.to(
          node,
          {
    ...
    mm.add("(max-width: 767px)", () => {
      ...
      // ── MOBILE ENTRANCE TIMELINE: Nav indicator + Buyer card enter together from right ──
      ...
      // STAGE 2: Supplier enters from LEFT → CENTER (0.10 to 0.35)
      tl.to(
        supplier,
        {
          x: 0,
      ...
      // STAGE 3: Visually connect the two sides with FIT FOUND in center (0.42 to 0.65)
      if (connection) {
        tl.to(
          connection,
          {
      ...
      // STAGE 4: Match Unlocked card appears IN THE CENTER (0.72 to 0.90)
      if (connection) {
        tl.to(
          connection,
          {
  ...
  return (
    <div className="relative w-full bg-paper overflow-hidden" data-two-sided-wrapper="true">
      ...
      <div className="hidden md:block w-full h-full relative overflow-hidden">
        {/* Top desktop section indicator: Section-level pinned progress pill */}
        <div
          ref={desktopIndicatorRef}
          className="absolute top-[72px] md:top-[80px] left-1/2 -translate-x-1/2 z-30 flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-paper/95 backdrop-blur-md border border-ink/[0.08] shadow-sm pointer-events-none whitespace-nowrap"
        >
          <span className="text-[10px] font-mono text-copper uppercase tracking-wider font-bold">
            05 — TWO-SIDED NETWORK
          </span>
      ...
      {/* ══════════════════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════════════════ */}
      <div className="flex md:hidden flex-col w-full h-full relative overflow-hidden justify-center px-4 pt-20 pb-4">
        {/* Mobile Indicator: Section-level pinned progress pill */}
        <div
          ref={mobileIndicatorRef}
          className="absolute top-[66px] sm:top-[72px] left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 px-3 py-1 rounded-full bg-paper/95 backdrop-blur-md border border-ink/[0.08] shadow-sm pointer-events-none whitespace-nowrap max-w-[calc(100vw-24px)]"
        >
          <span className="text-[9px] font-mono text-copper uppercase tracking-wider font-bold shrink-0">
            05 — TWO-SIDED NETWORK
          </span>
          ...
          <span className="w-1 h-1 rounded-full bg-ink/20 shrink-0" />
          <span className="text-[9px] font-mono text-slate/75 font-medium truncate">
            {activeStep === 0
              ? "1. BUYER ARRIVES"
              : activeStep === 1
              ? "2. SUPPLIER ARRIVES"
        ...
        >
          ...
          <Link
            href="/discover"
            className="group inline-flex items-center gap-1 mt-2 text-[11px] font-semibold text-copper hover:text-copper-muted transition-colors"
          >
            Find matches
            <ArrowRight size={10} weight="bold" className="group-hover:translate-x-0.5 transition-transform duration-200" />
          </Link>
        </div>
...
function SupplierContent({ isMatched = false }: { isMatched?: boolean }) {
  return (
    <div>
      ...
      <p className="mt-1.5 sm:mt-2 text-sm sm:text-base text-slate leading-relaxed max-w-[36ch]">
      ...
      {/* Supplier workflow roadmap */}
      <div className="mt-3 sm:mt-4 p-2.5 rounded-lg bg-ink/[0.03] border border-ink/[0.04]">
        <span className="text-[9px] font-mono text-slate/50 uppercase tracking-wider block mb-1">
          SUPPLIER PLATFORM WORKFLOW
        </span>
        <div className="flex items-center gap-1 text-[10px] font-mono text-slate/80 flex-wrap">
          ...
          <span className="text-copper">→</span>
          <span className="font-semibold text-copper">Relevant Requirements</span>
        </div>
      </div>

      <Link
        href="/register"
        className="group inline-flex items-center gap-1.5 mt-3 sm:mt-5 text-xs font-semibold text-copper hover:text-copper-muted transition-colors"
      >
        Get discovered
        <ArrowRight size={12} weight="bold" className="group-hover:translate-x-1 transition-transform duration-200" />
      </Link>
    </div>
  );
}
...
function BuyerContent({ isMatched = false }: { isMatched?: boolean }) {
  return (
    <div>
      ...
      <p className="mt-1.5 sm:mt-2 text-sm sm:text-base text-silver/90 leading-relaxed max-w-[36ch]">
      ...
      {/* Buyer workflow roadmap */}
      <div className="mt-3 sm:mt-4 p-2.5 rounded-lg bg-white/[0.04] border border-white/10">
        <span className="text-[9px] font-mono text-silver/50 uppercase tracking-wider block mb-1">
          BUYER SOURCING WORKFLOW
        </span>
        <div className="flex items-center gap-1 text-[10px] font-mono text-silver/80 flex-wrap">
          <span className="font-semibold text-surface">Requirement</span>
          <span className="text-copper">→</span>
          <span className="font-semibold text-surface">Qualified Suppliers</span>
          <span className="text-copper">→</span>
          <span className="font-semibold text-surface">Compare Fit</span>
          <span className="text-copper">→</span>
          <span className="font-semibold text-surface">Request Quotes</span>
          <span className="text-copper">→</span>
          <span className="font-semibold text-copper">Connect</span>
        </div>
      </div>
